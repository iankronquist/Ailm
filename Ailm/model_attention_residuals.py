...
class AilmAttentionResidualsConfig(AilmV1Config):
    '''
...
def _init_weights(name: str, module: nn.Module):
    '''
    Initialize the weights for the module.
    '''
    if isinstance(module, nn.Linear) and not name.endswith('_residual_projection'):
    ...
    elif isinstance(module, nn.Linear) and name.endswith('_residual_projection'):
        module.weight[:] = mlx.core.zeros_like(module.weight)
...
class Mlp(nn.Module):
    ...
    def __init__(self, config: AilmAttentionResidualsConfig):
        super().__init__()
...
class CausalSelfAttention(nn.Module):
    ...
    '''
    def __init__(self, config: AilmAttentionResidualsConfig, rope: nn.RoPE):
        super().__init__()
...
def attention_residual(blocks: list[Tensor], projection: nn.Linear) -> Tensor:
    V = mlx.core.stack(blocks)
    K = rms_norm(V)
    logits = mlx.core.einsum('d, nbtd -> nbt', projection.weight.squeeze(), K)
    h = mlx.core.einsum('nbt, nbtd -> btd', mlx.core.softmax(logits, 0), V)
    return h
class Layer(nn.Module):
    ...
    def __init__(self, config: AilmAttentionResidualsConfig, rope: nn.RoPE):
        ...
        self.attn_residual_projection = nn.Linear(config.hidden_size, 1, bias=False)
        self.mlp_residual_projection = nn.Linear(config.hidden_size, 1, bias=False)
    ...
    def __call__(self, blocks: list[Tensor]) -> list[Tensor]:
        '''
        TODO document
        https://github.com/MoonshotAI/Attention-Residuals/blob/master/Attention_Residuals.pdf
        '''

        h = attention_residual(blocks, self.attn_residual_projection)

        attn_out = self.attn(self.ln_1(h))
        blocks = blocks + [attn_out]

        h  = attention_residual(blocks, self.mlp_residual_projection)
...
class Transformer(nn.Module):
    def __init__(self, config: AilmAttentionResidualsConfig):
        ...
        self.h = [Layer(config, rope) for _ in range(config.layer_count)]
...
class AilmAttentionResiduals(AilmV1):

    def __init__(self, config: AilmAttentionResidualsConfig):
        '''
        Initialize the AilmAttentionResiduals model.
        '''
        ...
        self.output_residual_projection = nn.Linear(config.hidden_size, 1, bias=False)
    ...
    def __call__(self, ids: Tensor) -> Tensor:
        ...
        x = attention_residual(blocks, self.output_residual_projection)
