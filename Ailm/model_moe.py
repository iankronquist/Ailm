...
from model import AilmV1, AilmV1Config, _init_weights


@dataclasses.dataclass
class AilmMoeConfig(AilmV1Config):
    '''
    ...
    expert_k: int = 4
    moe_size: int = 768 * 4
    non_moe_stride: int = 2 # if (i % non_moe_stride) == 0 we will have a non-moe layer, otherwise an moe one.
...
class Mlp(nn.Module):
    ...
    def __init__(self, config: AilmV1Config, width: int):
        super().__init__()

        self.c_fc = nn.Linear(config.hidden_size, width, bias=False)
        self.activation = nn.SiLU()
        self.c_proj = nn.Linear(width, config.hidden_size, bias=False)
...
class Moe(nn.Module):
    def __init__(self, config: AilmMoeConfig):
        super().__init__()

        self.expert_k = config.expert_k
        self.score_experts = Mlp(config, config.expert_count)

        self.experts: list[Mlp] = [Mlp(config, config.moe_size) for _ in range(config.expert_count)]
 
    def __call__(self, x: Tensor) -> typing.Tuple[Tensor, Tensor]:
        
        scores = self.score_experts(x)
        score_indices = mlx.core.argpartition(scores, self.expert_k)
        top_score_indices = [:self.expert_k]
        results = []
        for expert_index in top_score_indices:
            expert_index = expert_index.item()
        ...
        return x, score_indices

class CausalSelfAttentionMoe(nn.Module):
    '''
    CausalSelfAttention
    
    We use Grouped Query Attention to save parameters: https://arxiv.org/pdf/2305.13245
    '''
    def __init__(self, config: AilmMoeConfig, rope: nn.RoPE):
        super().__init__()
...
class HyperConnection(nn.Module):
    def __init__(self, config: AilmMoeConfig, inner_block: nn.Module):
...
class LayerMoe(nn.Module):
    ...
    '''
    def __init__(self, config: AilmMoeConfig, rope: nn.RoPE, is_moe: bool):
        '''
        ...
        self.is_moe = is_moe
        self.attn = CausalSelfAttentionMoe(config, rope)
        self.ln_1 = rms_norm
        if is_moe:
            self.ffn = Moe(config)
        else:
            self.ffn = Mlp(config, config.mlp_size)
        self.ln_2 = rms_norm
    ...
    def __call__(self, x: Tensor, first_layer_value: typing.Optional[Tensor]) -> typing.Tuple[Tensor, typing.Optional[Tensor], typing.Optional[Tensor]]:
        '''
        ...
        ffn_result, ffn_scores = self.ffn(self.h2_pre(self.ln_2(x)))
        x = self.h2_res(x) + ffn_result

        return x, first_layer_value, ffn_scores
...
class TransformerMoe(nn.Module):
    def __init__(self, config: AilmMoeConfig):
        ...
        if config.hyper_connection_residual_stream_expansion_factor == 1:
            self.h = nn.Sequential(*[LayerMoe(config, rope, i % config.non_moe_stride == 0) for i in range(config.layer_count)])
        else:
            self.h = nn.Sequential(*[HyperConnection(config, LayerMoe(config, rope, i % config.non_moe_stride == 0)) for i in range(config.layer_count)])
...
class AilmMoe(AilmV1):
    ...
    def __init__(self, config: AilmMoeConfig):
        ...
        self.transformer = TransformerMoe(config)
    ...
    def __call__(self, ids: Tensor) -> Tensor:
        ...
        for layer in self.transformer.h.layers:
            x, first_layer_value, ffn_scores = layer(x, first_layer_value)
            all_ffn_scores.append(ffn_scores)
